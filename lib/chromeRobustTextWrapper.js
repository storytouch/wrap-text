...
class ChromeRobustTextWrapper {
  ...
  */
  ...
  calculateLineDimensions(context) {
    ...
    const y0 = visible ? this.totalHeight : -1;
  ...
  initializeWidthCalculator() {
    ...
    const isEqual = (fontA, fontB) => fontA.path === fontB.path;
    const distinctFontFilter = (type, index, self) => (
      self.findIndex((i) => isEqual(type.fontConfig, i.fontConfig)) === index
    );

    const fonts = Object.values(types)
      .filter((type) => !!type.fontConfig)
      .filter(distinctFontFilter)
      .map((type) => type.fontConfig);

    this.textWidthCalculator = new TextWidthCalculator(fonts);
  }
}
